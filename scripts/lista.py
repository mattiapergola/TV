...
def italy_channels():
    ...
    def save_as_m3u(channels, daddylive_channels=None, filename="italy.m3u"):
        ...
        def strip_cover_variant(name):
            value = normalize_cover_name(name)
            value = re.sub(r"-(\d+|d|mpd)$", "", value)
            return value

        def load_cover_maps():
            cover_maps_path = os.path.join(script_dir, "src", "channel-cover-maps.generated.json")
            try:
                with open(cover_maps_path, "r", encoding="utf-8") as f:
                    payload = json.load(f)
                return payload.get("ita_portrait_map", {}), payload.get("landscape_map", {})
            except Exception as e:
                print(f"[WARN] Impossibile leggere {cover_maps_path}: {e}")
                return {}, {}

        def resolve_cover_url(mapping, display_name):
            if not mapping:
                return ""
            exact = mapping.get(normalize_cover_name(display_name), "")
            if exact:
                return exact
            return mapping.get(strip_cover_variant(display_name), "")

        cover_portrait_map, cover_landscape_map = load_cover_maps()
        ...
        with open(os.path.join(output_dir, filename), "w", encoding="utf-8") as f:
            ...
            for category, channel_list in channels_by_category.items():
                ...
                for ch in channel_list:
                    ...
                    cover_portrait = resolve_cover_url(cover_portrait_map, name)
                    cover_landscape = resolve_cover_url(cover_landscape_map, name)
                    cover_portrait_attr = f' tvg-cover-portrait="{cover_portrait}"' if cover_portrait else ''
                    cover_landscape_attr = f' tvg-cover-landscape="{cover_landscape}"' if cover_landscape else ''

                    f.write(
                        f'#EXTINF:-1 tvg-id="{tvg_id}" tvg-logo="{logo}"'
                        f'{cover_portrait_attr}{cover_landscape_attr} group-title="{category}",{name}\n'
                    )
